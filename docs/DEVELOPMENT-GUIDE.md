...
- A BitGo **testnet** access token + a funded hot wallet + its passphrase for real on-chain sends.

## Local run

```bash
make install
cp .env.example .env        # fill BITGO_ACCESS_TOKEN, BITGO_WALLET_PASSPHRASE
make typecheck
make test
make build
